...
def test_people_get_outputs_person():
    respx.get("https://app.loxo.co/api/acme/people/5").mock(
        return_value=httpx.Response(200, json={"id": 5, "name": "Jane", "custom_text_3": "utm"})
    )
...
def test_people_create_wraps_person_and_merges_flags():
    ...
    result = runner.invoke(
        app,
        ["--json", "people", "create", "--name", "Jane", "--field", "custom_text_3=utm"],
        env=ENV,
    )
    assert result.exit_code == 0
...
def test_people_update_puts_person():
    ...
    result = runner.invoke(app, ["--json", "people", "update", "5", "--title", "VP"], env=ENV)
    assert result.exit_code == 0
