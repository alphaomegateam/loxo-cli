...
def test_candidates_list_under_job():
    respx.get("https://app.loxo.co/api/acme/jobs/3/candidates").mock(
        return_value=httpx.Response(200, json={
            "scroll_id": None, "candidates": [{"id": 1, "person_id": 50}]}))
    result = runner.invoke(
        app, ["--json", "candidates", "list", "--job", "3"], env=ENV)
...
def test_candidates_add_sends_person_id():
    ...
    respx.post("https://app.loxo.co/api/acme/jobs/3/candidates").mock(
        side_effect=handler)
    result = runner.invoke(
        app, ["--json", "candidates", "add", "--job", "3", "--person", "50"],
        env=ENV)
...
def test_candidates_update_highlights():
    ...
    respx.put("https://app.loxo.co/api/acme/jobs/3/candidates/9").mock(
        side_effect=handler)
    result = runner.invoke(
        app, ["--json", "candidates", "update", "9", "--job", "3",
              "--highlights", "Strong"],
        env=ENV)
    assert result.exit_code == 0
